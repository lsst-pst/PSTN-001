...
Counts $C_b$ have to be calibrated to be useful for science.  Traditional astronomical 
magnitudes are reported as a measurement relative to some judiciously chosen celestial 
calibration source (e.g., Vega),
\begin{equation}
\label{Vegamag}
...
it has exactly the same spectral energy distribution as the calibration source). This
clearly undesireable effect is in practice corrected for using the so-called ``color terms''. 
...
are straightforward and effectively there is no practical difference between the approaches.
When $F^{calib}_{b}$ is unavailable, $m^{Vega}_b$ are simply relative flux measurements
and cannot be transformed to an absolute flux scale. 
...
exception is SDSS, which used maggies. As discussed below, there appears
to be no major advantage for LSST to adopt maggies over Jansky. Therefore,
it is better to adopt the more commonly used Jansky. 
...
wise to give it preference over much less common maggies. For the faint fluxes
...
In summary, the proposal is to calibrate LSST fluxes to a physical scale (e.g. using
Gaia catalogs), adopt nJy as the flux unit, and report the best available estimates of 
$\Delta_{br}$ and $\Delta_r$ with each LSST Data Release. In particular, $\Delta_r$ 
